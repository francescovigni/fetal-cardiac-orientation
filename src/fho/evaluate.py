...
def bland_altman(pred, ref) -> dict:
    ...
    return dict(bias_deg=bias, sd_deg=sd,
...
def stratify(errors, covariate, edges, name="covariate") -> str:
    ...
    for lo, hi in zip(edges[:-1], edges[1:]):
        ...
        if k.sum() == 0:
            continue
        lines.append(f"    {lo:6.2f}–{hi:6.2f}  n={k.sum():3d}  "
...
def risk_coverage(errors, confidence, steps=10) -> str:
    ...
    order = np.argsort(-c)
    e = e[order]
    lines = ["  risk-coverage (abstain on the least confident):",
             "    coverage  median   p90"]
...
def report(pred, ref, covariates: dict | None = None,
           confidence=None, title="") -> str:
...
def _predict(ckpt: Path, raw: Path, split: str, source: str) -> dict:
    ...
    return dict(pred=preds, ref=refs, aniso=aniso, size=size,
                conf=conf, disagree=disagree)
...
def main(a):
    ...
    if a.ckpt and Path(a.ckpt).exists():
        ...
        print(report(r["pred"], r["ref"],
                     covariates={"roundness b/a": (r["aniso"], [0.0, 0.6, 0.75, 0.9, 1.01]),
                                 "heart semi-major (px)": (r["size"], [0, 60, 90, 120, 400])},
                     confidence=r["conf"],
                     title=f"landmark model / {a.split} / crops from {a.source}"))
        print()
        print(report(r["pred"], r["ref"], confidence=-np.array(r["disagree"]),
